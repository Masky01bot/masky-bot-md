...
bot(
	...
	async (message, match) => {
		...
		if (!isImAdmin) return await message.send(`*You need the role of an admin to perform this task.* ~_Admin_~`)
		let user = message.mention[0] || message.reply_message.jid
		if (!user && match != 'all') return await message.send(`*Give me a user,Let make this Fun* ~User~`)
		const isUserAdmin = match != 'all' && (await isAdmin(participants, user))
		if (isUserAdmin) return await message.send(`*HURRAY am an admin.* Admin✅`)
		if (match == 'all') {
			user = participants
...
bot(
	...
	async (message, match) => {
		...
		if (!match) return await message.send('Example : add 2348067333655')
		// if (!match.startsWith('@@')) {
...
bot(
	...
	async (message, match) => {
		...
		if (!user) return await message.send(`*Give me a user,Let make this Fun* ~User~`)
...
bot(
	...
	async (message, match) => {
		...
		if (!user) return await message.send(`*Give me a user,Let make this Fun* ~User~`)
		const isUserAdmin = await isAdmin(participants, user)
		if (!isUserAdmin) return await message.send(`*You need the role of an admin to perform this task.* ~_Admin_~`)
...
bot(
	...
	async (message, match) => {
		...
		if (!im) return await message.send(`*You need the role of an admin to perform this task.* ~_admin_~`)
