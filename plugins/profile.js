...
bot(
  ...
  async (message, match) => {
    ...
    if (!id) return await message.send('*Give me a user,Let make this Fun* ~User~')
    await message.send('*HURAY YOU HAVE SUCCESSFULLY BLOCK THIS USER*,_USER BLOCKRD✅_')
...
bot(
  ...
  async (message, match) => {
    const id = message.mention[0] || message.reply_message.jid || (!message.isGroup && message.jid)
    if (!id) return await message.send('**Give me a user,Let make this Fun* ~User~')
    await message.send('*HURAY YOU HAVE SUCCESSFULLY UNBLOCK THIS USER*,_USER UNBLOCKED✅_')
